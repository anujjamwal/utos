...
pCtrlBlock processes[2];
volatile int pInd = 0;
volatile pCtrlBlock * context;



void initContext(pCtrlBlock * pcb) {
...
void tick(void) __attribute__ ((naked));
void tick(void) {
    SAVE_CONTEXT();

    pInd = (pInd + 1) & 0x01;
    context = &processes[pInd];
